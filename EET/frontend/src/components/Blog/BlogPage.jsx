...
//           <Link to="/blog" className="text-orange-500 hover:underline" style={{ color: highlightColor }}>
//             Back to Blogs
//           </Link>
//         </div>
//       </div>
...
//                   style={{ backgroundColor: highlightColor, width: "60px", height: "60px" }}
//                 >
...
//               {/* Blog Content - Full Content from DB */}
//               <div ref={contentRef} className="prose max-w-none text-sm sm:text-base" dangerouslySetInnerHTML={{ __html: blog.text }} />
//             </div>
...
//   const commentRef = useRef(null);
//   const [blog, setBlog] = useState(null);
//   const [recentBlogs, setRecentBlogs] = useState([]); // Dynamic recent posts
//   const [loading, setLoading] = useState(true);
//   const [error, setError] = useState(null);
//   const [commentData, setCommentData] = useState({
//     name: "",
...
//     // Fetch recent blogs (latest 3)
//     const fetchRecentBlogs = async () => {
//       try {
//         const response = await axios.get("http://localhost:5000/api/blogs");
//         setRecentBlogs(response.data.slice(0, 3)); // Take first 3 (latest from backend sort)
//       } catch (err) {
...
//       fetchBlog();
//     }
//     fetchRecentBlogs(); // Always fetch recent
...
//                   <Link key={post._id} to={`/blog/${post._id}`} className="flex items-center gap-3 sm:gap-4 block hover:bg-gray-50 p-2 rounded-lg transition-colors">
//                     <img
//                       src={post.image}
//                       alt={post.title}
...
//                       <h4 className="text-gray-900 font-semibold text-sm sm:text-base line-clamp-2 truncate">{post.title}</h4>
//                       <span className="text-gray-500 text-xs sm:text-sm">{post.date}</span>
//                     </div>
//                   </Link>
//                 ))}
...
const BlogPage = () => {
  ...
  const [blog, setBlog] = useState(null);
  const [recentBlogs, setRecentBlogs] = useState([]); // Dynamic recent posts
  const [comments, setComments] = useState([]); // Approved comments
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [commentData, setCommentData] = useState({
    name: "",
  ...
  // EmailJS credentials (same as Contact)
  const EMAILJS_SERVICE_ID = "service_3y5kn1o";
  const EMAILJS_TEMPLATE_ID = "template_08pxfxd"; // Ensure template includes {{approve_url}}, {{reject_url}}, {{type}} variables in dashboard
  const EMAILJS_PUBLIC_KEY = "6_OgNNuq3l_GlsbSo";
  const API_BASE_URL = "http://localhost:5000/api"; // Adjust if needed
  const DOMAIN_URL = "http://localhost:3000"; // Your frontend domain for any redirects if needed
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/comments/blog/${id}?status=approved`);
    ...
    if (id) {
      fetchBlog();
      fetchComments(); // Fetch comments for the blog
    }
    fetchRecentBlogs(); // Always fetch recent
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      // Step 1: POST comment to backend as 'pending'
      const newComment = {
        blogId: id,
      ...
      const postResponse = await axios.post(`${API_BASE_URL}/comments`, newComment);
      const commentId = postResponse.data._id;

      // Step 2: Prepare approve/reject URLs (backend endpoints)
      const approveUrl = `${API_BASE_URL}/comments/${commentId}/approve`; // Backend should handle approval and optional redirect
      const rejectUrl = `${API_BASE_URL}/comments/${commentId}/reject`; // Backend should handle reject to 'trash' status

      // Step 3: Collect form data for EmailJS with dynamic URLs
      const formValues = {
        from_name: commentData.name,
        from_email: commentData.email,
        user_phone: "", // Not used for comments
        user_subject: `New Comment on Blog: ${blog.title} - Awaiting Approval`,
        user_message: `New comment received on blog: ${blog.title}\n\nCommenter Name: ${commentData.name}\nEmail: ${commentData.email}\nWebsite: ${commentData.website || 'Not provided'}\nComment: ${commentData.comment}\n\nBlog Link: ${DOMAIN_URL}/blog/${id}\n\nApprove or Reject below:`,
        approve_url: approveUrl, // Placeholder for email template button/link
        reject_url: rejectUrl,   // Placeholder for email template button/link
        type: "comment", // For template conditional (e.g., {{#if type}} for comment-specific content)
      };

      // Log for debugging
      console.log("Comment Form Values:", formValues);

      // Step 4: Send email via EmailJS
      const result = await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, formValues);
      console.log("Comment email sent successfully:", result.text);
      setSubmitted(true);
      ...
      // Optionally refetch comments if approved immediately, but since pending, wait for email action
  ...
  return (
    <div ref={sectionRef} className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 sm:py-12 px-4 sm:px-6 md:px-8 overflow-x-hidden mt-20">
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
          {/* Main Content */}
          <div className="lg:col-span-2">
            ...
            {comments.length > 0 && (
              <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">Comments ({comments.length})</h2>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
                            <h4 className="font-semibold text-gray-900 text-sm sm:text-base">{comment.name}</h4>
                            <span className="text-xs sm:text-sm text-gray-500 mt-1 sm:mt-0">{new Date(comment.createdAt).toLocaleDateString()}</span>
                          </div>
                          {comment.website && (
                            <p className="text-xs text-blue-600 mb-2">
                              <a href={comment.website} target="_blank" rel="noopener noreferrer">Website</a>
                            </p>
                          )}
                          <p className="text-sm sm:text-base text-gray-700">{comment.comment}</p>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div ref={commentRef} className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
              ...
              <form onSubmit={handleCommentSubmit} className="space-y-4 sm:space-y-6">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg hover:bg-orange-600 transition-colors duration-300 font-semibold cursor-pointer text-sm sm:text-base disabled:opacity-50 disabled:cursor-not-allowed"
                  style={{ backgroundColor: highlightColor }}
                >

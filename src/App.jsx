...
// Helper: Simple JWT decode to check validity (expiry)
...
// Strict Protected Route - Blocks render until check completes
...
const ProtectedRoute = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (!valid) {
      console.log('Invalid or no token. Redirecting to /login from:', location.pathname);
  ...
  // Always show loader first to prevent direct render
  if (isLoading) {
    return (
  ...
  // If not auth, don't render children (redirect already triggered)
  if (!isAuthenticated) {
    return null;
  }

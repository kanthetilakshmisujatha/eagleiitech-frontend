...
const JobApplicationPages = () => {
  ...
  const fetchJob = async () => {
    try {
      ...
      const foundJob = response.data.find((j) => j.title === decodeURIComponent(title));
      setJob(foundJob);
    } catch (error) {
  ...
  const fetchRecentBlogs = async () => {
    try {
      ...
      setRecentPosts(response.data.slice(0, 3).map((blog) => ({
        id: blog._id,
        title: blog.title,
        date: new Date(blog.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }).toUpperCase(),
        image: blog.image,
      })));
    } catch (error) {
  ...
  const handleChange = (e) => {
    const { name, value, files } = e.target;
    if (name === 'cvResume' && files && files[0]) {
      const file = files[0];
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.cvResume) {
      submissionData.append('cvResume', formData.cvResume); // File attach avtundi
    }

    try {
      // Log FormData for debug (won't show file content, but fields)
      console.log('Submitting FormData:');
      for (let [key, value] of submissionData.entries()) {
        console.log(key, value);
      }

      // POST to backend (resume attach avtundi email lo)
      ...
      // Detailed logging
      console.error("Full AxiosError:", error);
      console.error("Error Response:", error.response?.data);
      console.error("Error Status:", error.response?.status);
      console.error("Error Message:", error.message);

      // User-friendly alert with details
      const errorMsg = error.response?.data?.error || error.message || "Unknown error";
      alert(`Application submit cheyadam lo problem: ${errorMsg}. Console lo full details chudu.`);
    } finally {
  ...
  if (loading) {
    return <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">Loading...</div>;
  }

  if (!job) {
    return <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">Job not found.</div>;
  }

  const truncatedDescription = job.description.length > 200 ? `${job.description.substring(0, 200)}...` : job.description;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 sm:py-12 px-4 sm:px-6 md:px-8 overflow-x-hidden mt-10">
          ...
          <div className="lg:col-span-2">
            <div ref={formRef} className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 md:p-8">
              <div className="mb-4 sm:mb-6">
                <div className="flex items-center mb-2">
                  <FaUserShield className="mr-2 text-lg sm:text-xl" style={{ color: '#F54A00' }} />
                  <h3 className="text-lg sm:text-xl font-semibold" style={{ color: '#F54A00' }}>
                ...
                <p className="mt-2 text-gray-600 text-sm sm:text-base">
                  ...
                  <span>{expanded ? job.description : truncatedDescription}</span>
                  {!expanded && job.description.length > 200 && (
                    <button
                      onClick={() => setExpanded(true)}
                      className="ml-2 text-orange-500 underline text-sm hover:text-orange-600"
                    >
                      Read More
                    </button>
                  )}
                ...
                <p className="mt-2 text-gray-600 text-sm sm:text-base">
                  <strong>Job Category:</strong> {job.role}
                </p>
                <p className="text-gray-600 text-sm sm:text-base">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                ...
                <div>
                  ...
                  <div className="mt-1 flex justify-center px-4 sm:px-6 pt-4 sm:pt-5 pb-5 sm:pb-6 border-2 border-gray-300 border-dashed rounded-lg">
                    <div className="space-y-1 text-center">
                      ...
                      <p className="text-xs text-gray-500">PDF, DOC, DOCX up to 20MB</p>
                    </div>
                  </div>
                  {formData.cvResume && (
                    <p className="text-sm text-gray-600 mt-3 flex items-center">
                ...
                <button
                  ...
                  className="w-full text-white py-2 sm:py-3 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors duration-200 font-medium text-sm sm:text-lg disabled:opacity-50 disabled:cursor-not-allowed"
                  style={{ backgroundColor: '#F54A00' }}
                  onMouseOver={(e) => !submitLoading && (e.target.style.backgroundColor = '#E04400')}
                  onMouseOut={(e) => !submitLoading && (e.target.style.backgroundColor = '#F54A00')}
                >
            ...
            <div ref={sidebarRef} className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 sticky top-16">
              ...
              <div className="space-y-4">
                ...
                {recentPosts.map((post) => (
                  <div 
                    key={post.id} 
                    className="flex items-center gap-3 sm:gap-4 cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-colors duration-200"
                  ...
                  >
                    ...
                    />
                    <div className="flex flex-col">
                      <h4 className="text-gray-900 font-semibold text-sm sm:text-base line-clamp-2">{post.title}</h4>
                      <span className="text-gray-500 text-xs sm:text-sm">{post.date}</span>
                    </div>
                  </div>
                ))}
      ...
      {submitted && (
        <div className="fixed top-5 right-5 bg-green-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg shadow-lg z-50">
          Application successfully submitted with resume attached!
        </div>
      )}
    </div>
  );
